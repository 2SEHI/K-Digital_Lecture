...
class DeepSARSA(tf.keras.Model):
    ...
    # Model을 훈련할 때 자동으로 호출되는 메소드
    # 이 메소드에서 실제 층을 생성(쌓는다)
    def call(self, x):
        x = self.fc1(x)
        x = self.fc2(x)
        q = self.fc_out(x)
...
class DeepSARSAgent:
    def __init__(self, state_size, action_size):
        ...
        # 최적화 함수 설정
        # keras 버전에 따라 lr을 사용하면 경구
        # leraning rate를 사용하라고 함
...
if __name__ == "__main__":
    ...
    # 오차값과 에피소드를 저장할 리스트를 생성
    scores, episodes = [], []
    # 훈련할 횟수 - 시간이 오래 걸리면 조정가능
    EPISODES = 1000
